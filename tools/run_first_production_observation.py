...
A replay divergence is retained as a result. This tool never upgrades a capture beyond
OBSERVATION and never interprets a hidden-state trajectory as mechanism evidence.
"""
...
def _assert_exact_experiment_request(request: Mapping[str, Any], *, require_receipts: bool) -> dict[str, Any]:
...
def _exclusive_write_json(path: Path, value: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        raise CaptureContractError(f"refusing to overwrite existing artifact {path}") from exc
...
def prepare(output: Path) -> str:
    ...
    final_request = _assert_exact_experiment_request(final_request, require_receipts=True)
...
def _run_capture(
    ...
    if len(lines) != 1 or len(lines[0]) != 64 or any(ch not in "0123456789abcdef" for ch in lines[0]):
        raise CaptureContractError("capture CLI returned a malformed manifest SHA-256 receipt")
    return lines[0]


def _read_bundle(directory: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    return tuple(_read_json(directory / name) for name in BUNDLE_FILES)  # type: ignore[return-value]


def _verify_observation_bundle(directory: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
...
def _file_receipts(directory: Path) -> dict[str, str]:
    ...
    for name in BUNDLE_FILES:
        try:
            ...
            raise CaptureContractError(f"unable to hash {directory / name}: {exc}") from exc
...
def observe(
    ...
    request = _assert_exact_experiment_request(_read_json(request_path), require_receipts=True)
    ...
    equality = {name: receipts_a[name] == receipts_b[name] for name in BUNDLE_FILES}
