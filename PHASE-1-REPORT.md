...
`MATH-SPEC.md` now freezes stable `GEO-MATH-001` through `GEO-MATH-011` definitions covering finite trajectories, finite differences, path length, the project cosine convention, alignment, Menger curvature, transformation laws, undefined cases, and the exact/numerical boundary.
...
The two Codex review rounds now fix and regression-test:
...
- scale-aware significant-digit result normalization that preserves tiny nonzero values;
...
The hardened suite contains **41 unit tests**. GitHub Actions independently reruns the suite on Python 3.11, 3.12, and 3.13 and verifies both frozen metadata identities and byte-for-byte result identity.
...
Future mathematical changes must produce a new release identity and update/reprove affected `GEO-LEAN-TGT-*` theorems rather than rewriting the frozen record.
