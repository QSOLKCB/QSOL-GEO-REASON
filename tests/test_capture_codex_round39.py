...
from qsol_geo_reason.capture_hub_tree import CANONICAL_HF_ENDPOINT, prepare_tree_receipts
...
class CaptureRound39RegressionTests(unittest.TestCase):
    def test_online_warmup_generates_qsol_tree_artifact_and_pins_canonical_endpoint(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            seen_api_endpoints: list[str | None] = []
            seen_snapshot_endpoints: list[str | None] = []

            class Api:
                def __init__(self, *, endpoint=None):
                    seen_api_endpoints.append(endpoint)
            ...
            def fake_snapshot_download(**kwargs):
                seen_snapshot_endpoints.append(kwargs.get("endpoint"))
                return str(snapshots / kwargs["revision"])
            ...
            with (
                ...
                mock.patch.dict("os.environ", {"HF_ENDPOINT": "https://mirror.invalid"}),
            ):
                receipts = prepare_tree_receipts(request)

            self.assertEqual(seen_api_endpoints, [CANONICAL_HF_ENDPOINT])
            self.assertTrue(seen_snapshot_endpoints)
            self.assertEqual(
                set(seen_snapshot_endpoints),
    ...
    def test_schema_requires_nonnegative_cuda_index_and_null_elsewhere(self):
        ...
        for rule in rules:
            ...
            then_index = rule.get("then", {}).get("properties", {}).get(
                "cuda_resolved_device_index"
            )
            else_index = rule.get("else", {}).get("properties", {}).get(
                "cuda_resolved_device_index"
            )
    ...
    def test_custom_signal_handler_is_rejected(self):
        ...
        with (
            mock.patch("signal.valid_signals", return_value={sigint, sigterm}),
            mock.patch("signal.getsignal", side_effect=getsignal),
            mock.patch("signal.getitimer", return_value=(0.0, 0.0)),
        ):
            with self.assertRaisesRegex(
                CaptureContractError, "custom asynchronous signal handler"
            ):
    ...
    def test_armed_interval_timer_is_rejected(self):
        ...
        timer_values = [
            value
            for value in (
                getattr(signal, "ITIMER_REAL", None),
                getattr(signal, "ITIMER_VIRTUAL", None),
                getattr(signal, "ITIMER_PROF", None),
            )
        ...
        with (
            mock.patch("signal.valid_signals", return_value={sigint}),
            mock.patch("signal.getsignal", return_value=signal.default_int_handler),
            mock.patch("signal.getitimer", side_effect=getitimer),
        ):
            with self.assertRaisesRegex(
                CaptureContractError, "armed asynchronous interval timer"
            ):
                _assert_no_async_signal_instrumentation()
    ...
    def test_cuda_library_receipt_is_embedded_in_authenticated_runtime_config(self):
        ...
        config = (
            'SYNTHETIC\nQSOL_GEO_CUDA_RUNTIME={"loaded_cuda_runtime_libraries":'
            '{"cuda_runtime_library_file_count":2,"cuda_runtime_library_receipt_sha256":"'
            + "a" * 64
            + '"}}\n'
        )
        observed = {
