...
class CaptureRound45RegressionTests(unittest.TestCase):
    def test_every_production_lane_requires_fresh_torch_and_transformers_imports(self):
        for device in ("cpu", "mps", "cuda:0"):
            ...
            with self.assertRaisesRegex(CaptureContractError, "fresh PyTorch/Transformers"):
                Round45Backend._assert_pristine_mps_import_state(
                    device, modules={"torch.library": object()}
                )
            with self.assertRaisesRegex(CaptureContractError, "fresh PyTorch/Transformers"):
                Round45Backend._assert_pristine_mps_import_state(
                    device, modules={"transformers.models.synthetic": object()}
                )
    ...
    def test_cpu_flush_denormal_policy_is_content_bound_in_build_receipt(self):
        ...
        self.assertIn("QSOL_GEO_CPU_FLUSH_DENORMAL=false\n" + _CPU_RUNTIME, recorded)
