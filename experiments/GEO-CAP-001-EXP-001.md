...
Use the frozen Python 3.11 capture-reference lane from a clean checkout. The virtual environment must be created **outside the repository checkout** so environment files cannot dirty the source tree that preparation later authenticates. The selected reference interpreter is fixed at `/tmp/qsol-geo-reason-capture-py311/bin/python`. The canonical procedure deliberately **does not source `activate`** and does not rely on `VIRTUAL_ENV` or activation-time PATH mutation; every environment-management command names that interpreter directly. Create the external environment, install the exact CPU PyTorch build under the complete runtime lock, install the capture extra under that same lock, then verify the installed closure before preparation:

```bash
python3.11 -m venv /tmp/qsol-geo-reason-capture-py311
/tmp/qsol-geo-reason-capture-py311/bin/python -m pip install \
...
/tmp/qsol-geo-reason-capture-py311/bin/python -m pip install \
  ...
  -e '.[capture]' \
  'jsonschema==4.23.0'
/tmp/qsol-geo-reason-capture-py311/bin/python -B tools/verify_capture_reference_environment.py
/tmp/qsol-geo-reason-capture-py311/bin/python -m pip check
/tmp/qsol-geo-reason-capture-py311/bin/python --version  # must report Python 3.11.x
```

`constraints/capture-reference-py311.txt` is the authoritative complete resolved runtime lock for this Python 3.11 Linux x86_64 CPU reference lane. It pins the direct capture packages **and their transitive runtime closure**, including Torch dependencies, Transformers/Hugging Face dependencies, Requests/TLS dependencies, and the jsonschema validator closure. `tools/verify_capture_reference_environment.py` rejects a missing pin, version mismatch, unexpected non-bootstrap runtime distribution, non-CPython interpreter, non-Linux/x86_64 platform, or any interpreter outside Python 3.11. The fixed-interpreter `--version` line is only a human-readable sanity check; the machine verifier enforces the interpreter requirement itself.
...
Define the following Bash function once from the repository root using Bash's `function` reserved word with `qsol_first_observation` as the function name. Because `function` introduces a function definition rather than a simple-command position, a pre-existing alias named `qsol_first_observation` is not expanded while the definition is parsed. Invoke the resulting function only as `\qsol_first_observation ...`; quoting the command word with the backslash suppresses alias expansion at call time before the function enters its loader-scrub boundary. **Before starting any Python process or helper executable**, the outer subshell first enables Bash POSIX mode using the pure assignment `POSIXLY_CORRECT=1`; this does not perform command lookup. In POSIX mode, Bash special builtins are resolved before shell functions. The pre-interpreter boundary therefore uses only backslash-escaped special builtins: `unset` to clear every environment variable whose name begins `LD_`, `DYLD_`, `_RLD_`, or `LDR_`, plus `GLIBC_TUNABLES`, `LIBPATH`, and `SHLIB_PATH`; `exit` to fail closed with status 126 if any removal fails or any loader control survives; and finally `exec` to cross into the clean shell. There is no pre-authentication `local`, `printf`, `command`, `python`, or other ordinary command lookup for an ambient shell function to intercept. The backslashes also prevent definition-time alias substitution of the special-builtin words.

After the verified scrub, `\exec /bin/bash --noprofile --norc -p -c ...` replaces the subshell with the fixed Bash executable. Because `exec` is a POSIX special builtin at this boundary, an ambient function named `exec` cannot intercept it; the executable pathname supplied to `exec` is dispatched as the external program rather than as an ambient function named `/bin/bash`. The privileged, no-profile/no-rc Bash child does not import exported shell functions or startup aliases. The fixed external reference interpreter pathname `/tmp/qsol-geo-reason-capture-py311/bin/python` is passed as a **literal positional argument** to that clean child; the wrapper does not read `VIRTUAL_ENV`, PATH activation state, or another ambient selector when choosing the first Python process. The authenticated Python bootstrap program is likewise passed as a literal positional argument on the same `exec` command line; it is never assigned to, read from, or validated through an ambient shell variable. Only after the function-free boundary has been established does the clean child copy positional argument `$1` into its private `qsol_python` variable and `$2` into its private `qsol_bootstrap` variable. Consequently a pre-existing, exported, or readonly `VIRTUAL_ENV` or `QSOL_FIRST_OBSERVATION_BOOTSTRAP` is irrelevant and cannot replace either the selected interpreter or the trusted `-c` program. The child requires the fixed Python pathname to be absolute and a regular executable file before invoking that exact path directly. Consequently ambient functions named `python`, `command`, `exec`, `/bin/bash`, `unset`, or `exit`, as well as definition-time aliases for the special builtins, cannot receive the authenticated bootstrap invocation. Only then does the inline `-I -S -B -c` program run. That program is independent of the checkout: it resolves fixed system Git, requires a root-owned/non-group-or-world-writable executable on POSIX, hashes that executable before and after use, invokes Git with a minimal redirection-free environment, resolves one immutable `HEAD^{commit}` **before** reading the launcher, reads `<that-commit>:tools/run_first_production_observation.py`, computes the SHA-256 of those exact committed launcher bytes, and injects both the resolved commit and launcher digest into the committed launcher namespace. The working-tree launcher bytes are never loaded by this first boundary, and a later movement of `HEAD` cannot change the revision attributed to the already-running launcher.
