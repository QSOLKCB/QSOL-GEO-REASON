"""Round-57 hardening for sealed source identity and transient runtime changes."""
...
def _git_child_environment_round57() -> dict[str, str]:
...
def _make_sealed_provenance_entrypoints():
    # All provenance helpers share this private globals dictionary. Git lookups made
    # by the cloned call graph therefore resolve the private runner below rather than
    # the caller-writable ``qsol_geo_reason.provenance._git_run`` module slot.
    ...
    for name, value in tuple(private_globals.items()):
        if (
            ...
            and value.__module__ == _provenance.__name__
        ):
            clones[name] = _clone_provenance_function(value, private_globals)
    private_globals.update(clones)
    private_globals["_git_run"] = _git_run_round57
    return clones["git_source_revision"], clones["resolve_implementation_revision"]


(
    _git_source_revision_round57,
    _resolve_implementation_revision_round57,
) = _make_sealed_provenance_entrypoints()
del _make_sealed_provenance_entrypoints

# Keep the compatibility slot pointed at the hardened runner, but canonical source
# identity no longer trusts this mutable binding: the public entrypoints above use a
# private cloned call graph with its own runner binding.
_provenance._git_run = _git_run_round57
_provenance.git_source_revision = _git_source_revision_round57
_provenance.resolve_implementation_revision = _resolve_implementation_revision_round57
...
class _DarwinMappedLibraryRound57(_round56._DarwinMappedLibrary):
...
def _macho_uuid_and_executable_segments(
...
def _sha256_mapped_executable_segments(
...
def _mapped_macho_identity_round57(header_address: int, slide: int) -> tuple[str, str]:
    try:
    ...
    uuid, segments = _macho_uuid_and_executable_segments(
        commands, endian=endian, ncmds=ncmds
    )
    return uuid, _sha256_mapped_executable_segments(slide, segments)


def _sha256_fd_executable_segments(
    fd: int,
    *,
    base_offset: int,
    slice_size: int,
...
def _thin_macho_identity_from_fd_round57(
    ...
    return uuid, _sha256_fd_executable_segments(
        fd,
...
def _macho_identities_from_fd_round57(fd: int) -> frozenset[tuple[str, str]]:
    try:
        info = os.fstat(fd)
    ...
    if prefix[:4] in _round56._MACHO_MAGICS:
        value = _thin_macho_identity_from_fd_round57(fd, 0, file_size)
    ...
    for index in range(nfat):
        ...
        value = _thin_macho_identity_from_fd_round57(
...
def _darwin_loaded_library_paths_round57(
    predicate: Any,
) -> list[Path | _DarwinMappedLibraryRound57]:
    try:
    ...
    result: dict[tuple[str, str, str], _DarwinMappedLibraryRound57] = {}
    ...
    for index in range(count):
        ...
        if not predicate(candidate):
            continue
        mapped_uuid, mapped_digest = _mapped_macho_identity_round57(
            int(header), int(image_slide(index))
        )
        key = (candidate, mapped_uuid, mapped_digest)
        result.setdefault(
            key,
            _DarwinMappedLibraryRound57(
...
def _runtime_library_measurement_round57(
    raw_path: Any,
    *,
    predicate: Any,
    label: str,
    ...
    if isinstance(raw_path, _round56._DarwinMappedLibrary) and not isinstance(
        raw_path, _DarwinMappedLibraryRound57
    ):
        raise CaptureContractError(
            f"mapped {label} Darwin runtime image lacks a cryptographic mapped-code receipt"
        )
    if not isinstance(raw_path, _DarwinMappedLibraryRound57):
    ...
    try:
        ...
        if expected_identity not in _macho_identities_from_fd_round57(fd):
            raise CaptureContractError(
...
_cuda_runtime._darwin_loaded_library_paths = _darwin_loaded_library_paths_round57
_cpu_runtime._darwin_loaded_library_paths = _darwin_loaded_library_paths_round57
_cuda_runtime._runtime_library_measurement = _runtime_library_measurement_round57
...
def _package_file_stat_fingerprint(path: Path) -> tuple[int, ...]:
    try:
        link = os.lstat(path)
        target = os.stat(path)
    except OSError as exc:
...
def _torch_package_stability_receipt(
    module: Any,
    ...
    for path in paths:
        ...
        entries.append((rel, _package_file_stat_fingerprint(path)))
    if not entries:
        raise CaptureContractError(
...
def _make_torch_package_stability_vault():
...
(
    _remember_round57_torch_package_stability,
    _recall_round57_torch_package_stability,
) = _make_torch_package_stability_vault()
del _make_torch_package_stability_vault
...
def _initialize_round57_torch_package_baseline(self: Any) -> None:
    _ROUND56_INITIALIZE_TORCH_PACKAGE_BASELINE(self)
    if self._real_round56_torch_runtime():
        _remember_round57_torch_package_stability(
            self, _torch_package_stability_receipt(self._torch)
        )


def _assert_round57_torch_package_provenance(self: Any) -> None:
    _ROUND56_ASSERT_TORCH_PACKAGE_PROVENANCE(self)
    baseline = _recall_round57_torch_package_stability(self)
    if baseline is None:
        if self._real_round56_torch_runtime():
            raise CaptureContractError(
        ...
        return
    if _torch_package_stability_receipt(self._torch) != baseline:
        raise CaptureContractError(
...
# the same class identity while its construction/observation checks gain Round-57
# stability semantics before capture_execute freezes the trusted callable surface.
_Round56Backend._initialize_round56_torch_package_baseline = (
    _initialize_round57_torch_package_baseline
)
_Round56Backend._assert_round56_torch_package_provenance = (
    _assert_round57_torch_package_provenance
)
