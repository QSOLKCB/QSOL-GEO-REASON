...
- the preparation receipt binds the canonical Hub endpoint, immutable model/tokenizer tree receipts, and the complete measured reference environment;
...
- the replay verdict binds both the semantic preparation-receipt hash and its raw archived file hash, which transitively binds the nested reference-environment receipt into successful replay evidence.
